...
import { Mic, MicOff, ChevronRight, Image, X, User, Activity, Stethoscope, Brain, Check, AlertTriangle } from 'lucide-react';
...
const inputCls = 'w-full bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-gray-400 focus:outline-none transition-colors';
const labelCls = 'text-xs text-gray-500 font-medium tracking-wide uppercase';
...
export default function NewCase() {
  ...
  return (
    <div className="p-8 max-w-xl">
          ...
          return (
            <div key={s.label} className="flex items-center gap-1.5">
              <div className={`flex items-center gap-1.5 text-xs font-medium ${
                i === step ? 'text-gray-900' : i < step ? 'text-emerald-600' : 'text-gray-300'
              }`}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center ${
                  i === step ? 'bg-gray-900 text-white' : i < step ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-100 text-gray-300'
                }`}>
        ...
        <Card className="p-5 space-y-4">
          {isExistingPatient ? (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 text-sm text-emerald-700 flex items-center gap-2">
              <Check size={14} /> {language === 'ha' ? 'Ana amfani da majiyyaci da aka yi rajista.' : 'Using existing registered patient.'}
            </div>
          ) : (
            <>
              <Input
      ...
      {step === 1 && (
        <Card className="p-5 space-y-4">
          <p className={labelCls}>{language === 'ha' ? 'Alamomin Rayuwa (zaɓi)' : 'Vital Signs (optional)'}</p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { key: 'bp_systolic',     label: language === 'ha' ? 'Matsin Jini (Systolic)' : 'BP Systolic (mmHg)',  placeholder: '120', type: 'number' },
              { key: 'bp_diastolic',    label: language === 'ha' ? 'Matsin Jini (Diastolic)' : 'BP Diastolic (mmHg)', placeholder: '80',  type: 'number' },
              { key: 'temperature',     label: language === 'ha' ? 'Zafin Jiki (°C)' : 'Temperature (°C)',           placeholder: '37.0', type: 'number', step: '0.1' },
              { key: 'pulse',           label: language === 'ha' ? 'Bugun Zuciya (bpm)' : 'Pulse (bpm)',             placeholder: '80',  type: 'number' },
              { key: 'respiratory_rate',label: language === 'ha' ? 'Numfashi (/min)' : 'Resp. Rate (/min)',          placeholder: '18',  type: 'number' },
              { key: 'spo2',            label: 'SpO2 (%)',                                                            placeholder: '98',  type: 'number' },
            ].map(({ key, label, placeholder, type, step: s }) => (
              <div key={key} className="space-y-1.5">
                <label className={labelCls}>{label}</label>
                <input type={type} step={s} placeholder={placeholder} value={vitals[key]}
                  onChange={e => patchVitals(key, e.target.value)} className={inputCls} />
              </div>
            ))}
          </div>
          ...
          {vitals.bp_systolic >= 140 && (
            <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-xs text-red-700 flex items-center gap-2">
              ...
              {language === 'ha' ? 'Hawan jini mai tsanani — yi la\'akari da preeclampsia' : 'High BP detected — consider preeclampsia screening'}
            </div>
          )}
            ...
            <div className="grid grid-cols-1 gap-2">
              {DANGER_CHECKLIST.map(d => (
                <label key={d.id} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border cursor-pointer transition-colors ${
                  ...
                  <input type="checkbox" checked={!!checklist[d.id]} onChange={() => toggleCheck(d.id)} className="accent-red-600" />
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {recording && (
              <span className="text-xs text-red-500 animate-pulse flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-red-500 inline-block" /> Recording...
              </span>
            )}
        ...
        <Card className="p-5 space-y-4">
          <div className="space-y-3">
            <div className="bg-gray-50 rounded-xl p-4">
              <p className={`${labelCls} mb-1`}>{language === 'ha' ? 'Majiyyaci' : 'Patient'}</p>
            ...
            {vitalsText() && (
              <div className="bg-gray-50 rounded-xl p-4">
                <p className={`${labelCls} mb-1`}>{language === 'ha' ? 'Alamomin Rayuwa' : 'Vitals'}</p>
                <p className="text-sm text-gray-800">{vitalsText()}</p>
              </div>
            )}
            {Object.values(checklist).some(Boolean) && (
              <div className="bg-red-50 rounded-xl p-4 border border-red-100">
                <p className="text-xs text-red-400 mb-2 font-medium tracking-wide uppercase">{language === 'ha' ? 'Alamomin Haɗari' : 'Danger Signs Reported'}</p>
            ...
            {symptoms && (
              <div className="bg-gray-50 rounded-xl p-4">
                <p className={`${labelCls} mb-1`}>{language === 'ha' ? 'Ƙarin Bayani' : 'Additional Notes'}</p>
                <p className="text-sm text-gray-800 leading-relaxed">{symptoms}</p>
              </div>
            )}
            {image && (
              <div className="bg-gray-50 rounded-xl p-4">
                <p className={`${labelCls} mb-2`}>{language === 'ha' ? 'Hoto' : 'Clinical Image'}</p>
          ...
          <div className="flex justify-between pt-2">
            ...
            <Btn onClick={handleSubmit} disabled={loading} variant="green">
              {loading
                ? <><Spinner /> {language === 'ha' ? 'Ana nazari...' : 'Analysing...'}</>
                : <><Brain size={14} /> {language === 'ha' ? 'Gudanar da AI' : 'Run AI Analysis'}</>
              }
            </Btn>
          </div>
        </Card>
